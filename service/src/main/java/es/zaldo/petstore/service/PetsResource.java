 ...
 *
 * <p>This resource will be used to adds and return pets given different parameters.</p>
 */
@Path("/api/v1/pets/")
...
public class PetsResource {
    ...
    public Response getPet(@PathParam("id") final String id,
        ...
        try {
            ...
            if (debug) {
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Load_Pet_Core, monitor.getShortData(AvailableMonitors.Load_Pet_Core));
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Marshal_Pet, monitor.getShortData(AvailableMonitors.Marshal_Pet));
            }
    ...
    public Response updatePet(JSONObject jsonObject,
            @QueryParam("debug") final boolean debug) {
        try {
            ...
            ResponseBuilder builder = Response.status(Status.OK).entity(json);
            if (debug) {
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Unmarshal_Pet, monitor.getShortData(AvailableMonitors.Unmarshal_Pet));
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Update_Pet_Core, monitor.getShortData(AvailableMonitors.Update_Pet_Core));
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Marshal_Pet, monitor.getShortData(AvailableMonitors.Marshal_Pet));
            }
     ...
     *
     * @param ids pets IDs. Optional parameter, the default value is null.
     *
    ...
    public Response getPetsById(
            @QueryParam("id") @DefaultValue("") final List<String> ids,
            @QueryParam("debug") final boolean debug) {
        try {
            ...
            ResponseBuilder builder = Response.status(Status.OK).entity(json);
            if (debug) {
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Get_Pets_Core, monitor.getShortData(AvailableMonitors.Get_Pets_Core));
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Marshal_List, monitor.getShortData(AvailableMonitors.Marshal_List));
            }
            return builder.build();
        } catch (Exception e) {
            LOGGER.error(e);
     ...
     *
     * @param nwLatitude    Northwest latitude. Mandatory parameter.
     * @param nwLongitude   Northwest longitude. Mandatory parameter.
     * @param seLatitude    Southeast latitude. Mandatory parameter.
     * @param seLongitude   Southeast longitude. Mandatory parameter.
     * @param group     pet group. Optional parameter, the default value is null.
     * @param type      pet type. Optional parameter, the default value is null.
     * @param page Pagination start page. Default value is 0
     * @param size Num of rows to retrieve. Default value is 10
     *
     * @return Response containing a list of pet objects.
     */
    @GET
    ...
    public Response searchWithin(
        ...
        Box bounds = new Box(
                new Location(nwLatitude, nwLongitude),
                new Location(seLatitude, seLongitude));
        ...
        if (!pageRequestValidator.isValid(pagination)) {
            throw new DataValidationException(
                getInvalidPaginationErrorMessage());
        }
        ...
        try {
            ...
            ResponseBuilder builder = Response.status(Status.OK).entity(json);
            if (debug) {
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Within_Core, monitor.getShortData(AvailableMonitors.Within_Core));
                builder.header(PetsResource.DEBUG_PREFIX + AvailableMonitors.Marshal_List, monitor.getShortData(AvailableMonitors.Marshal_List));
            }
            return builder.build();
        } catch (Exception e) {
    ...
    private String getInvalidPaginationErrorMessage() {
        return MSG_MAX_PAGE + pageRequestValidator.getMaxPage() + ", " +
            MAX_PAGE_SIZE + pageRequestValidator.getMaxSize();
    }
